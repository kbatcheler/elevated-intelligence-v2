...
- [Cost/token telemetry honesty](cost-telemetry-honesty.md) — model_usage rows only for REAL billed calls; the billed flag separates no-call (record nothing) from billed-failure (record at cost); sum retry tokens; tap in orchestrator only.
